...
describe('Сторінка Реєстрації', () => {
    ...
    it('повинна мати коректні інпути і проходити реєстрацію', () => {
        ...
        cy.get('.NewAssemblyPage-module__select').should('have.length', 6);
        cy.get('.NewAssemblyPage-module__button').should('be.visible').and('contain', 'Зберегти збірку');

        cy.get('.NewAssemblyPage-module__select').first().select('Intel Core i5-11400');
        cy.get('.NewAssemblyPage-module__select').eq(1).select('16 GB DDR4');
        cy.get('.NewAssemblyPage-module__select').eq(2).select('Intel UHD Graphics');
        cy.get('.NewAssemblyPage-module__select').eq(3).select('SSD 512 GB');
        cy.get('.NewAssemblyPage-module__select').eq(4).select('MSI B460M PRO');
        cy.get('.NewAssemblyPage-module__select').eq(5).select('650W');
